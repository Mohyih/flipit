...
const Dashboard = ({ apiCall, navigateToEditor, navigateToQuiz, user }) => {
    ...
    const handleManageClick = (e, setId) => {
        e.stopPropagation();
        navigateToEditor(setId, handleSetUpdated);
    };
    ...
    // NEW: Callback to update Dashboard when SetEditor updates title/description
    const handleSetUpdated = (updatedSet) => {
        setSets(prevSets => prevSets.map(s => s.set_id === updatedSet.set_id ? updatedSet : s));
    };
