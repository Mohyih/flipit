...
const SetEditor = ({ apiCall, set, refreshSet, navigateToDashboard }) => {
  ...
  // Refresh current set when component mounts or refreshSet changes
  useEffect(() => {
    refreshSet();
  }, [refreshSet]);
  ...
  const handleUpdateSet = async () => {
    ...
    try {
      ...
      navigateToDashboard(); // ✅ Only go back when updating title/desc
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleAddCard = async () => {
    ...
    try {
      ...
      setCards(prev => [...prev, newCard]); // ✅ Stay on page
      await refreshSet();
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleDeleteCard = async cardId => {
    ...
    try {
      ...
      setCards(prev => prev.filter(card => card.card_id !== cardId)); // ✅ Stay on editor
      await refreshSet();
    } catch (err) {
      console.error(err);
    } finally {
