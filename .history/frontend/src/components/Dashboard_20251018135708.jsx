...
const Dashboard = ({ apiCall, navigateToEditor, navigateToQuiz, user }) => {
    ...
    useEffect(() => {
        const fetchSets = async () => {
            try {
                // When fetching sets, we rely on the API to return the 'description' field now
        ...
        fetchSets();
    }, [apiCall]); // Runs only on initial mount, unless 'apiCall' changes

    // Corrected handleDeleteSet in src/components/Dashboard.jsx
    const handleDeleteSet = async (setId) => {
        // 🛑 REMOVED: window.confirm() for instant deletion.
        try {
            await apiCall(`/sets/${setId}`, 'DELETE');
            // Optimistically update the UI after a successful deletion
