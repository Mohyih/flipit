// src/components/SetEditor.jsx (UPDATED WITH DESCRIPTION)
...
const SetEditor = ({ apiCall, set, refreshSet, navigateToDashboard }) => {
    ...
    const [description, setDescription] = useState(set.description || ''); // NEW
    ...
    const handleSaveTitle = async (e) => {
        ...
        try {
            if (isNewSet) {
                await apiCall('/sets', 'POST', { title: title.trim(), description: description.trim() }); // UPDATED
                console.log("Set saved! You can now add cards.");
                navigateToDashboard();
            } else {
                await apiCall(`/sets/${set.set_id}`, 'PUT', { title: title.trim(), description: description.trim() }); // UPDATED
                await refreshSet();
    ...
    return (
        <div className="set-editor">
            ...
            <form onSubmit={handleSaveTitle} style={{ textAlign: 'center', marginBottom: '30px' }}>
                ...
                <input
                    type="text"
                    placeholder="Set Description (optional)" // NEW
                    value={description}
                            ...
                            {set.cards.map((card, index) => (
                                <tr key={card.card_id}>
                                    ...
                                    <td>
                                        {editingCardId === card.card_id ? (
                                            <>
                                                <button onClick={() => handleSaveEdit(card.card_id)} className="action-button quiz-btn">
                                                    Save
                                                </button>
                                            ...
                                            <>
                                                <button onClick={() => handleEdit(card)} className="card-action-icon gear-icon" title="Edit Card">
                                                    &#x2699;
                                                </button>
                                                <button onClick={() => handleDeleteCard(card.card_id)} className="card-action-icon delete-icon" title="Delete Card">
                                                    &times;
                                                </button>
                                            </>
                                        )}
